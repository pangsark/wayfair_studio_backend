...
NANO_MODEL = os.getenv("NANO_BANANA_MODEL", "nanoai/nano-banana-pro")
...
def _call_annotator(page_path: Path) -> Path:
    # TODO: fix the replicate api call (model name, prompt)
    """Send the page image to the Nano Banana Pro model and return a path to
    the annotated image that we can feed into the CV stage.

    If the REPLICATE_API_TOKEN is not set we simply copy the original image
    (effectively bypassing the AI pass).
    """
    if not os.getenv("REPLICATE_API_TOKEN"):
        # no token, skip the AI pass
        return page_path
...
def _extract_bounding_boxes(orig_path: Path, annot_path: Path) -> List[Tuple[int, int, int, int]]:
    """Detect the coloured rectangles drawn by the annotator.  Uses simple
    image subtraction so the algorithm doesn't care what hue the boxes are.
...
def _process_pdf(path: Path, manual_id: int) -> int:
    """Main worker: convert PDF, call AI, crop steps, update DB.
...
def start_manual_processing(file_path: Path, name: str = None, slug: str = None, description: str = None) -> str:
    """Public helper invoked by the FastAPI endpoint.  Starts a background
    thread that performs the heavy work and returns a job_id immediately.
    """
    job_id = str(uuid.uuid4())
    manual_id = _create_manual_record(name=name, slug=slug, description=description)

    _JOBS[job_id] = {"status": "processing", "manual_id": manual_id, "step_count": 0, "error": None}
